...
public class DataBindTests {
    ...
    public void json2MapTest() throws Exception {
        ...
        String response = mockMvc.perform(post("/bind/j2m").contentType(MediaType.APPLICATION_JSON).content(data)).andExpect(status().is4xxClientError()).andReturn().getResponse().getContentAsString();
        log.info("response : [{}]", response);
    }
}
