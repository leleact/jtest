...
public class RequestBodyControllerTests {
    ...
    public void debugRequestBodyTest() throws Exception {
        ...
        request.setName("a");
        request.setAge(1);
        mockMvc.perform(post("/requestBody/common").contentType(MediaType.APPLICATION_JSON_VALUE).content(objectMapper.writeValueAsString(request))).andExpect(status().is2xxSuccessful()).andExpect(content().string("hello a"));
    }
}
